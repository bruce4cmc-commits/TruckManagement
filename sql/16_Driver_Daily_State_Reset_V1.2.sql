-- ============================================================================
-- 卡車循環運輸管理系統 V1.2 - Migration 16: DRIVER 換天狀態初始化與當日任務隔離 Patch
-- 檔案名稱: 16_Driver_Daily_State_Reset_V1.2.sql
-- 說明: 修正 get_driver_home() RPC，確保司機首頁與車況狀態嚴格限定於當日 (plan_date = current_date)
--       當日無任務顯示 NO_TRIP (今日無排程)；當日有任務但尚未發車顯示 DAY_START_READY (準備楊梅廠出廠)
-- ============================================================================
...
begin
  ...
  -- 3. 優先搜尋今日指派給該司機的 Trip (僅限 plan_date = current_date，狀態為 WAITING 或 RUNNING)
  ...
  if v_current_trip.trip_id is not null then
    ...
    -- 若無直接指派司機之未完成 Trip，嘗試搜尋預設車輛今日未完成之 Trip
    select * into v_current_trip
    from public.trip_plan
  ...
  -- 6. 狀態與 Event 衍生 (嚴格限制在當日 current_trip 之 effective events)
  if v_current_trip.trip_id is null then
    ...
    -- 查詢當日當趟 effective events 數量與最後一筆時間
    ...
    if v_event_count = 0 then
      -- 狀況 B: 今日 current trip 尚無點報任何 Event ➔ 準備楊梅廠出廠
      v_current_status := 'DAY_START_READY';
      v_next_event_code := 'YM_OUT';
      v_last_event_code := null;
      v_last_event_time := null;
    else
      -- 狀況 C: 當前趟次已有 Event，取最後一筆 Event 資訊
